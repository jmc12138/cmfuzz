...
### 步骤 2.4 ⬜ 跨语言差分：Go crypto（子进程桥）+ RustCrypto/ring（FFI）
- 统一"测试向量编码"（算法 id + 输入槽），C 链接、Rust FFI、Go 子进程共用同一输入。

### 步骤 2.5 ⬜ FHE：接 OpenFHE 与 SEAL 做 BFV 跨库差分；SEAL 扩 CKKS（误差界 oracle）

---

## 阶段 3（P2）：生态扩展
- libgcrypt / NSS(freebl) / nettle 静态库或独立靶；
- BouncyCastle(Java) / PyCryptodome / pyca 子进程桥（跨语言差分再扩）；
...
- [~] 2.2 差分算法扩展(SHA-3/KDF/公钥…)——已接 SHA-3 家族(op5–8: SHA3-256/512、SHAKE128/256)、
      KDF(op9 HKDF-SHA256、op10 PBKDF2-HMAC-SHA256)、公钥(op11 Ed25519 签名、op12 X25519)，
      及 **ECDSA-P256 验签互操作(op13)**——因 RSA/ECDSA 密钥结构化且 ECDSA k 随机化、签名字节
      不可比，改用验签互操作 oracle：参照端签名(约半数篡改)，后端只回 1 字节接受/拒绝判定，
      五方判定一致(op13 一万一千向量、seed 42/777)。待续 **RSA-PSS 验签互操作(op14)**。
      AEAD 模式(CCM/OCB/GCM-SIV)因 OpenSSL/后端支持不齐暂缓
- [ ] 2.3 PQClean PQC 跨库差分
- [ ] 2.4 Go/Rust 跨语言差分
- [ ] 2.5 OpenFHE 差分 + SEAL CKKS
- [ ] 阶段3 生态扩展
- [ ] 阶段4 ZK/Tink/其他语言
