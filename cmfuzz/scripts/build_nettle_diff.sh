...
#
# Requires the nettle development headers (Debian/Ubuntu: nettle-dev). Ed25519 /
# X25519 live in libhogweed (GMP-backed), so both -lhogweed and -lnettle are
# needed. Builds the OpenSSL reference runner (if absent) and the standalone
# nettle compute CLI. Passing any argument also builds the fault-injected variant.
set -euo pipefail
...
if command -v pkg-config >/dev/null 2>&1 && pkg-config --exists hogweed 2>/dev/null; then
  ...
  # Some distros' hogweed.pc omits -lnettle (nettle is a private requirement), so
  # append it explicitly — nettle_* symbols are referenced directly by our CLI.
  NET_LIBS="$(pkg-config --libs hogweed) -lnettle -lgmp"
fi
