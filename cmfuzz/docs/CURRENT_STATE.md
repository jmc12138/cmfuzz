...
端到端跑通：**9 个实现库 × 5 类原语 + PQC(262 规格) + FHE**，0 误报，且有故障注入自测证明 oracle 有效。
BoringSSL / aws-lc（符号冲突）+ wolfCrypt（原生 wc_* API），均走**独立子进程差分**（主控 OpenSSL 做参照）。
...
### 2.11 阶段 2.1 —— 子进程差分扩库（BoringSSL / aws-lc / wolfCrypt）+ 专属 L3 误用靶【新】
PLAN 2.1 起把差分从 4 库扩到更多库。BoringSSL/aws-lc 重定义 OpenSSL 符号，**无法同进程链接**；
wolfCrypt 用自己的 wc_* API（不走 OpenSSL 兼容层）。三者一律采用**独立子进程靶**架构
（`harness/subproc/`）：
...
| **L3 序列/误用(O6)** | AES-256-GCM：灾难性 nonce 复用、AEAD 未验证明文释放；ECDSA-P256 签名 nonce(k) 复用→私钥恢复；ML-KEM-768 密钥混淆免虚假协商；AES-256-CBC：可预测/复用 IV；EVP 上下文 use-after-free；**BoringSSL + aws-lc EVP_AEAD + wolfCrypt AES-GCM：nonce 复用、release-before-verify** | ✅ |
| 差分 | 同算法跨 4 库输出一致性（同进程）；**BoringSSL + aws-lc + wolfCrypt 独立子进程差分**（`diff_subproc`，5 算法 vs OpenSSL 四方一致） | ✅ |
...
## 4. 自测（证明"0 违反"不是空转）
`tests/negative_tests.sh` ✅ **24/24**（含差分库 + BoringSSL + aws-lc + wolfCrypt；只到 aws-lc 21；只到 BoringSSL 18；仅差分库 15；最小 14）：
BoringSSL / aws-lc / wolfCrypt 各贡献 **L3 nonce 复用 + release-before-verify + 子进程 DIFF_mismatch**
