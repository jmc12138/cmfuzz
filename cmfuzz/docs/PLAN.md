...
- ✅ **libgcrypt（GnuPG）子进程桥**（`harness/subproc/compute_libgcrypt.c`）：独立 C 实现，
  op0–8/10（hash/HMAC/AEAD/SHA3/SHAKE/PBKDF2）；HKDF（1.9.x 缺）与公钥 op11–14→NA。
  libgcrypt↔OpenSSL 6000 向量全一致，fault 触发 `DIFF_mismatch`。
- ✅ **nettle（GnuTLS）子进程桥**（`harness/subproc/compute_nettle.c`）：独立 C 实现，
  op0–6/8–12（含 HKDF、Ed25519、X25519、SHAKE256）；SHAKE128（3.7 缺）与 op13/14→NA。
  nettle↔OpenSSL 6000 向量全一致，fault 触发 `DIFF_mismatch`。
- ✅ **BouncyCastle（Java）子进程桥**（`harness/bcbridge/CmfCompute.java`）：纯 Java 独立实现，
  复用 diff_subproc 线协议实现**全部 op0–14**（含 ECDSA-P256/RSA-PSS 验签、Ed25519、X25519）。
  **Java 成为第五条独立语言血统**（继 OpenSSL/Go/Rust/Python）。BouncyCastle↔OpenSSL 6000 向量
  全一致（seed 42/777 各 3000），fault（`CMF_JAVA_FAULT`）触发 `DIFF_mismatch`。BC provider jar
  按需下载并 sha256 校验（`libs/` gitignore），接入 build_all/negative_tests/Dockerfile（apt default-jdk）。
- ✅ **TFHE-rs（Zama，Rust FHE）同态整数正确性 oracle**（`harness/tfhebridge`）：TFHE/CGGI 方案，
  独立于 SEAL/OpenFHE 的 BFV/CKKS。因 TFHE 整数运算精确，oracle 为**等值蜕变检查**（区别于 CKKS 误差界）：
  加/减/乘 + 分配律 `Enc(a)*(Enc(b)+Enc(c))` 解密后与明文 wrapping 运算逐位相等。`--features fault`
  破坏一个结果→触发 `O_tfhe_int_correctness`。第三条 FHE 血统（Rust）。cargo 构建，artifact gitignore。
- ⬜ NSS(freebl) 静态库或独立靶；pyca 子进程桥（注：pyca 底层即 OpenSSL，差分意义有限）；
- ⬜ dalek 曲线差分；HElib（BGV/CKKS，C++，重构建暂缓）。

## 阶段 4（P3，探索）
- ✅ **ZK：arkworks Groth16 证明验证 / 电路一致性 oracle**（`harness/zkbridge`，BN254）：小型 R1CS
  电路证明 `a*b=c`（a/b 私有见证、c 公开输入），跑 Groth16 setup/prove/verify，校验正确验证器必满足
  的蜕变关系——**完备性** `verify([c], π)=accept`、**可靠性** `verify([c+1], π)=reject`（错误公开输入）
  与 `verify([c], π')=reject`（他证）。违背即 `O_zk_groth16_verify`。`--features fault` 反转完备性
  结果模拟坏验证器→触发 oracle。cargo 构建（arkworks 0.4，pin zeroize/zeroize_derive 以适配 Rust 1.83），
  artifact gitignore、Cargo.lock 入库。这是经典/FHE oracle 未覆盖的 bug 类。
- ⬜ Tink 高层 API 误用（天然契合 L3/O6）；
- ⬜ gnark（需 Go）、libsnark（C++，重依赖）、swift-crypto / Zig std.crypto —— 本机无 Go/Swift/Zig
  工具链，libsnark 依赖重，暂缓（见 CURRENT_STATE 工具链说明）。
...
- [~] 阶段3 生态扩展（PyCryptodome/libgcrypt/nettle/BouncyCastle/TFHE-rs✅；NSS/dalek/HElib 待做）
- [~] 阶段4 探索（arkworks Groth16 ZK oracle✅；Tink 待做；gnark/libsnark/swift/Zig 工具链受限）
