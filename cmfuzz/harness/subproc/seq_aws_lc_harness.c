/*
 * CMFuzz L3 (sequence / API-misuse) harness — aws-lc EVP_AEAD state machine.
 *
 * aws-lc inherits BoringSSL's one-shot EVP_AEAD API (EVP_AEAD_CTX_seal/_open),
 * so it shares that state machine's misuse hazards. Per PLAN 2.1, each new
 * differential library carries its own L3 target. Links ONLY aws-lc.
 *
 ...
 *          EVP_AEAD_CTX_open() returns 0 (and yields NO trustworthy plaintext)
 *          on a forged/tampered input; the contract is to use the output only
 *          when open() returns 1.
 *          O6-release-before-verify : delivering plaintext for a tampered
 *          ciphertext (open() failed) is a violation.
 *
 * Faults:
...
#define ALG "AES-256-GCM/aws-lc-seq"
#define NONCELEN 12
#define TAGLEN   16
