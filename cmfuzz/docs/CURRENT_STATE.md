...
### 2.15 阶段 2.4（续）—— 跨语言差分（RustCrypto vs OpenSSL）【新】
再加一支独立语言血统：新增 Rust 后端 `harness/rustbridge`，同样说子进程 runner（`diff_subproc`）
的线协议，但用 **RustCrypto crates + dalek**（`sha2`/`sha3`/`hmac`/`hkdf`/`pbkdf2`/`chacha20poly1305`/
`aes-gcm`/`ed25519-dalek`/`x25519-dalek`/`p256`/`rsa`）实现全部 15 个 op。RustCrypto 与 OpenSSL/Go
均无派生关系，是第三条独立实现血统，逐字节一致同样是真正的跨语言 O1 差分。
- **覆盖**：op0–14 全部，与 Go 后端一一对应。ECDSA 用 `p256::ecdsa`（`Signature::from_der` + SHA-256
  预哈希验签），RSA-PSS 用 `rsa::pss::VerifyingKey::<Sha256>`（salt 自动识别），与 op13/op14 的
  裸模数/SEC1 点编码约定一致。
- **构建**：`scripts/build_rust_diff.sh` 编 runner + Rust 后端；`--features fault` 出故障自测变体。
  Cargo.toml 里 pin 了 `zeroize=1.8.1`/`zeroize_derive=1.4.2`/`base64ct=1.6.0` 以兼容 Rust 1.83（MSRV）。
- 实测 **Rust↔OpenSSL 11000 向量全一致**（seed 42×6000 + 777×5000，覆盖全部 15 op），故障注入
  正确触发 `DIFF_mismatch`。三方（OpenSSL 参照 + Go + Rust）跨语言血统互证。

### 2.16 阶段 2.5 —— FHE 跨库差分（OpenFHE vs SEAL BFV）+ CKKS 误差界【新】
把 FHE 从"单库 metamorphic"升级为真正的跨实现 O1 差分，并补上 CKKS 近似算术专用 oracle。
- **BFV 跨库差分**（`harness/fhe_diff_harness.cpp`）：**OpenFHE** 与 **Microsoft SEAL** 两套
  互相独立的同态加密库，统一明文模 t=65537、多项式度 8192。对随机 a,b,c∈[0,t)：
  - add：Dec(Enc(a)+Enc(b)) == (a+b) mod t
  - mul：Dec(Enc(a)·Enc(b)) == (a·b) mod t（含 relinearize）
  - 分配律：Dec(Enc(a)·(Enc(b)+Enc(c))) == a·(b+c) mod t

  两库解密结果须彼此一致且都等于明文域结果。密文的 RNS 布局/密钥格式各库不同、加密又
  是随机化的，故与 PQC / 公钥验签互操作差分同思路——比对**解密后的明文**而非密文字节。
  分歧触发 `O1_fhe_bfv_interop`。
- **CKKS 误差界**（`harness/fhe_ckks_harness.cpp`）：CKKS 是近似方案，没有可精确比对的明文，
  等值 oracle 不成立。改用误差界 oracle：对随机实数 a,b（scale=2^40）
  |Dec(Enc(a) op Enc(b)) − (a op b)| 必须落在按内在编码误差留足余量的绝对界内
  （add 1e-2、mul 1.0+1e-3·|a·b|），越界即 `O2_ckks_error_bound`。
- **构建**：`scripts/build_openfhe.sh`（克隆 openfhe-development v1.2.3，只编静态库
  core/pke/binfhe，headers 装到 `libs/openfhe/install`）、`scripts/build_fhe_diff.sh`
  （链接 SEAL + OpenFHE 静态库），接入 `build_all.sh`。SEAL/OpenFHE 均按需构建、gitignore。
- **负向自测**：`negative_tests.sh` 新增两条，fault 变体（`CMF_FHE_FAULT`）分别令 BFV 一库
  结果错位、CKKS 结果越界，实测分别触发 `O1_fhe_bfv_interop` / `O2_ckks_error_bound`。
- 实测：BFV 差分 seed 42/777 各 1000 迭代 add/mul/分配律全一致；CKKS 同样 2×1000 迭代
  全在误差界内。

### 2.17 阶段 3 —— 生态扩展：PyCryptodome 跨语言差分（Python 独立血统）【新】
把跨语言差分从 OpenSSL/Go/Rust 三方扩到第四条独立血统 **Python/PyCryptodome**。
- **后端**（`harness/subproc/compute_pycryptodome.py`）：复用 stage 2.1 的 `diff_subproc`
  线协议（`<op> <hex>` → hex/NA/ERR），用 **PyCryptodome** 实现 op0–14。PyCryptodome 是
  自带 C/asm 核的**独立实现**（不是 OpenSSL 封装，区别于 pyca/cryptography），故逐字节一致
  是真正的跨语言 O1 差分。X25519（op12）PyCryptodome 不提供 → 回 `NA`（runner 跳过）。
- **覆盖**：SHA-256/512、HMAC、ChaCha20-Poly1305、AES-256-GCM、SHA3-256/512、SHAKE128/256、
  HKDF、PBKDF2、Ed25519 签名（rfc8032 确定性）、ECDSA-P256 验签（DER 编码）、RSA-PSS 验签
  （裸模数 n + e=65537、salt=32）。验签 op 比对 1 字节 accept/reject 判定。
- **构建/自测**：`scripts/build_py_diff.sh` 编 OpenSSL 参照 runner + 生成可执行 wrapper
  （`python3 compute_pycryptodome.py`）；fault 变体经 `CMF_PY_FAULT=1` 翻转首字节。接入
  `build_all.sh`、`negative_tests.sh`（新增一条 DIFF_mismatch 故障注入）、`Dockerfile`
  （apt python3-pip + pip 装 pycryptodome）。
- 实测 **PyCryptodome↔OpenSSL 6000 向量全一致**（seed 42×3000 + 777×3000），fault 变体
  正确触发 `DIFF_mismatch`。四方（OpenSSL/Go/Rust/Python）跨语言血统互证。

### 2.18 阶段 3 —— 生态扩展：libgcrypt + nettle 跨库差分（独立 C 血统）【新】
再加两条独立 C 实现，复用 stage 2.1 的 `diff_subproc` 线协议：
- **libgcrypt（GnuPG）**（`harness/subproc/compute_libgcrypt.c`）：句柄/S-表达式 API，实现
  op0–8（SHA-256/512、HMAC、ChaCha20-Poly1305、AES-256-GCM、SHA3-256/512、SHAKE128/256）
  与 op10（PBKDF2）。op9（HKDF，1.9.x 未提供）与公钥 op11–14→`NA`。系统 `libgcrypt20-dev`。
- **nettle（GnuTLS 核）**（`harness/subproc/compute_nettle.c`）：逐原语 ctx API，实现
  op0–6、8–12（含 HKDF、Ed25519、X25519、SHAKE256）。op7（SHAKE128，3.7 仅提供 SHA3-256-rate
  的 SHAKE256）与验签 op13/14→`NA`。Ed25519/X25519 在 libhogweed（GMP），链接 `-lhogweed -lnettle -lgmp`。
- **构建/自测**：`scripts/build_gcrypt_diff.sh`、`scripts/build_nettle_diff.sh` 编 OpenSSL 参照
  runner + 各自 compute CLI（fault 变体 `-DCMF_DIFF_FAULT=1`）。接入 `build_all.sh`、
  `negative_tests.sh`（各一条 DIFF_mismatch 故障注入）、`Dockerfile`（apt libgcrypt20-dev + nettle-dev）。
- 实测两库 **各自↔OpenSSL 6000 向量全一致**（seed 42/777 各 3000），fault 变体均触发
  `DIFF_mismatch`。至此差分覆盖 OpenSSL/BoringSSL/aws-lc/wolfCrypt/Botan/Go/Rust/Python/
  libgcrypt/nettle 共 10 个实现（含 4 条独立语言血统）。本阶段 0 新发现。

### 2.19 阶段 3 —— 生态扩展：BouncyCastle 跨语言差分（Java 独立血统，全 15 op）【新】
再加第五条独立语言血统 **Java/BouncyCastle**（纯 Java 实现，非 OpenSSL 封装）：
- **后端**（`harness/bcbridge/CmfCompute.java`）：复用 `diff_subproc` 线协议，用 BC 的 JCA
  provider + 轻量级 API 实现**全部 op0–14**——SHA-256/512、HMAC、ChaCha20-Poly1305、AES-256-GCM、
  SHA3-256/512、SHAKE128/256（`SHAKEDigest`）、HKDF（`HKDFBytesGenerator`）、PBKDF2
  （`PKCS5S2ParametersGenerator`，原始字节口令）、Ed25519（`Ed25519Signer`）、X25519
  （`X25519Agreement`）、ECDSA-P256 验签（`SHA256withECDSA`，DER）、RSA-PSS 验签
  （`SHA256withRSA/PSS`，salt=32、e=65537）。是唯一实现全 15 op 的额外后端。
- **构建/自测**：`scripts/build_java_diff.sh` 按需下载 pinned BC provider jar（bcprov-jdk18on
  1.78.1，sha256 校验，`libs/` gitignore）、`javac` 编译、生成 `java -cp ... CmfCompute` 可执行
  wrapper；fault 变体 `CMF_JAVA_FAULT=1`。接入 `build_all.sh`、`negative_tests.sh`（一条
  DIFF_mismatch 故障注入）、`Dockerfile`（apt default-jdk）。
- 实测 **BouncyCastle↔OpenSSL 6000 向量全一致**（seed 42/777 各 3000），fault 触发
  `DIFF_mismatch`。差分实现总数达 11，独立语言血统达 5（OpenSSL-C/Go/Rust/Python/Java）。本阶段 0 新发现。

### 2.20 阶段 3 —— 生态扩展：TFHE-rs 同态整数正确性 oracle（第三条 FHE 血统，Rust）【新】
在 stage 2.5 的 SEAL/OpenFHE（BFV/CKKS）之外再加一条独立 FHE 血统 **TFHE-rs（Zama，Rust）**：
- **oracle**（`harness/tfhebridge`）：TFHE/CGGI 方案，对加密 `FheUint32` 做同态加/减/乘与分配律
  `Enc(a)*(Enc(b)+Enc(c))`，解密后与明文 `wrapping_*` 结果**逐位相等**。因 TFHE 整数运算精确，这是
  等值蜕变检查（不同于 CKKS 的近似误差界 O2）。任何不等打印 `CMF_VIOLATION ... oracle=
  O_tfhe_int_correctness` 并以非零码退出，故兼作 fault 自测。
- **构建/自测**：`scripts/build_tfhe.sh` 用 cargo 编译（`tfhe = "0.8"`，features `integer,seeder_unix`；
  首次编译约 2 分钟；artifact `harness/tfhebridge/target/` gitignore，Cargo.lock 入库锁版本）；
  `--features fault` 破坏一个乘法结果→触发 oracle。接入 `build_all.sh`；`negative_tests.sh` 仅在
  已构建时跑（基础镜像不编译 TFHE-rs，故 SKIP，属按需重库，同 SEAL/OpenFHE 策略）。
- 实测明文运算精确一致（乘法含自举，约 8s/迭代），fault 变体触发 `O_tfhe_int_correctness`。
  FHE 血统达 3（SEAL/OpenFHE/TFHE-rs）。本阶段 0 新发现。

### 2.21 阶段 4 —— 探索：arkworks Groth16 zk-SNARK 验证/电路一致性 oracle（BN254）【新】
首次进入 ZK 领域——经典/FHE oracle 未覆盖的证明系统 bug 类：
- **oracle**（`harness/zkbridge`）：小型 R1CS 电路证明 `a*b=c`（a/b 私有见证、c 公开输入），
  用 arkworks Groth16（BN254）跑 setup/prove/verify，校验正确验证器必满足的蜕变关系：
  **完备性** `verify([c], π)=accept`；**可靠性** `verify([c+1], π)=reject`（错误公开输入）与
  `verify([c], π')=reject`（另一语句的证明）。任一违背打印 `CMF_VIOLATION ... oracle=
  O_zk_groth16_verify` 并非零退出。
- **构建/自测**：`scripts/build_zk.sh` 用 cargo 编译（arkworks 0.4；因 arkworks 最新传递依赖
  zeroize/zeroize_derive 要 Rust≥1.85（edition2024），pin 为 `=1.8.1`/`=1.4.2` 以适配仓库统一的
  Rust 1.83；artifact `harness/zkbridge/target/` gitignore、Cargo.lock 入库锁版本）；`--features fault`
  反转完备性结果模拟坏验证器→触发 oracle。接入 `build_all.sh`；`negative_tests.sh` 仅在已构建时跑
  （基础镜像不编译，故 SKIP，属按需重库策略）。
- 实测完备性+可靠性关系全部成立，fault 变体触发 `O_zk_groth16_verify`。gnark(需 Go)/libsnark(C++重依赖)/
  swift-crypto/Zig 因本机无对应工具链暂缓（PLAN 阶段4 已标注）。本阶段 0 新发现。
...
| 差分 | 同算法跨 4 库输出一致性（同进程）；**BoringSSL + aws-lc + wolfCrypt + Botan 独立子进程差分**（`diff_subproc`，op0–14＝哈希/HMAC/AEAD + SHA-3/SHAKE + HKDF/PBKDF2 + Ed25519/X25519 + ECDSA-P256/RSA-PSS 验签互操作，vs OpenSSL 五方一致）；**PQC 跨库 O1 差分 liboqs↔PQClean**（ML-KEM-768 KEM 互操作 + ML-DSA-65 验签互操作，双向）；**跨语言 O1 差分 Go crypto↔OpenSSL + RustCrypto↔OpenSSL**（`diff_subproc` go/rust 后端，op0–14 全覆盖，两条独立语言血统） | ✅ |
...
## 4. 自测（证明"0 违反"不是空转）
`tests/negative_tests.sh` ✅ **30/30**（含差分库 + BoringSSL + aws-lc + wolfCrypt + Botan + PQClean + Go + Rust；
无 Rust 时 29；无 Go+Rust 时 28；只到 Botan 27；只到 wolfCrypt 24；只到 aws-lc 21；只到 BoringSSL 18；仅差分库 15；最小 14）：
BoringSSL / aws-lc / wolfCrypt / Botan 各贡献 **L3 nonce 复用 + release-before-verify + 子进程 DIFF_mismatch**
三项，PQClean 贡献 **PQC 跨库差分 O1_kem_interop**，Go 与 Rust 各贡献 **跨语言差分 DIFF_mismatch**
...
## 6. 变更记录
- 2026-07-09（夜·18）：**PLAN 阶段4（探索·arkworks Groth16 ZK oracle）** —— 首次进入 ZK。新增
  `harness/zkbridge`（BN254 R1CS `a*b=c` 电路，Groth16 setup/prove/verify，校验完备性+两条可靠性
  蜕变关系，`O_zk_groth16_verify`）与 `scripts/build_zk.sh`（cargo，pin zeroize/zeroize_derive 适配
  Rust 1.83，`--features fault` 反转完备性）。实测关系全部成立、fault 触发 oracle；接入 build_all，
  negative_tests 按需自测。gnark/libsnark/swift/Zig 工具链受限暂缓。本阶段 0 新发现。
- 2026-07-09（夜·17）：**PLAN 阶段3（生态扩展·TFHE-rs 同态整数正确性 oracle）** —— 加第三条独立
  FHE 血统 TFHE-rs（Zama，Rust）。新增 `harness/tfhebridge`（`FheUint32` 加/减/乘 + 分配律，解密
  与明文 wrapping 逐位相等，`O_tfhe_int_correctness`）与 `scripts/build_tfhe.sh`（cargo，
  `--features fault` 破坏结果）。实测明文精确一致，fault 触发 oracle；接入 build_all，negative_tests
  按需自测（基础镜像 SKIP）。FHE 血统达 3。本阶段 0 新发现。
- 2026-07-09（夜·16）：**PLAN 阶段3（生态扩展·BouncyCastle 跨语言差分）** —— 加第五条独立语言
  血统 Java。新增 `harness/bcbridge/CmfCompute.java`（BC JCA + 轻量级 API，全 op0–14，唯一实现
  全 15 op 的额外后端）与 `scripts/build_java_diff.sh`（按需下载+sha256 校验 BC jar、javac 编译、
  生成 wrapper，`CMF_JAVA_FAULT` 故障变体）。**BouncyCastle↔OpenSSL 6000 向量全一致**（seed 42/777），
  fault 触发 `DIFF_mismatch`；negative_tests 加一条，Dockerfile 装 default-jdk。差分实现总数 11，
  独立语言血统 5。本阶段 0 新发现。
- 2026-07-09（夜·15）：**PLAN 阶段3（生态扩展·libgcrypt + nettle 跨库差分）** —— 再加两条独立
  C 实现。新增 `harness/subproc/compute_libgcrypt.c`（GnuPG，op0–8/10）与 `compute_nettle.c`
  （GnuTLS，op0–6/8–12 含 HKDF/Ed25519/X25519/SHAKE256），`scripts/build_gcrypt_diff.sh` /
  `build_nettle_diff.sh`。两库各↔OpenSSL **6000 向量全一致**（seed 42/777），fault 均触发
  `DIFF_mismatch`；negative_tests 各加一条，Dockerfile 装 libgcrypt20-dev + nettle-dev。差分实现
  总数达 10（含 4 条独立语言血统）。本阶段 0 新发现。
- 2026-07-09（夜·14）：**PLAN 阶段3（生态扩展·PyCryptodome 跨语言差分）** —— 把跨语言差分
  扩到第四条独立血统 Python/PyCryptodome。新增 `harness/subproc/compute_pycryptodome.py`
  （复用 diff_subproc 线协议，实现 op0–14，X25519 不支持→NA），`scripts/build_py_diff.sh`
  编 runner + 生成 wrapper（`CMF_PY_FAULT` 出故障变体）。**PyCryptodome↔OpenSSL 6000 向量
  全一致**（seed 42/777 各 3000）；negative_tests 新增一条 DIFF_mismatch 故障注入；Dockerfile
  装 python3-pip + pycryptodome。四方（OpenSSL/Go/Rust/Python）互证。本阶段 0 新发现。
- 2026-07-09（夜·13）：**PLAN 阶段2.5（FHE 跨库差分 OpenFHE vs SEAL BFV + CKKS 误差界）** ——
  把 FHE 从单库 metamorphic 升级为真正的跨实现 O1 差分。新增 `harness/fhe_diff_harness.cpp`
  （OpenFHE↔SEAL BFV，统一 t=65537/度 8192，add/mul/分配律解密结果两库互证＋对明文域取模）
  与 `harness/fhe_ckks_harness.cpp`（SEAL CKKS 误差界 oracle，scale 2^40）。`scripts/build_openfhe.sh`
  按需克隆+编 OpenFHE v1.2.3 静态库，`scripts/build_fhe_diff.sh` 链接 SEAL+OpenFHE，接入
  build_all.sh。负向自测新增两条（fault 触发 `O1_fhe_bfv_interop`/`O2_ckks_error_bound`）。
  实测 BFV/CKKS 各 seed 42/777×1000 迭代全通过。cmake 走 pip 装（VM 无系统 cmake/clang）。
  本阶段 0 新发现。
- 2026-07-09（夜·12）：**PLAN 阶段2.4（续·跨语言差分 RustCrypto vs OpenSSL）** —— 再加一支
  独立语言血统。新增 Rust 后端 `harness/rustbridge`（RustCrypto crates + dalek），复用 `diff_subproc`
  线协议，实现全部 15 个 op（op0–14），与 Go 后端一一对应。`scripts/build_rust_diff.sh` 编 runner +
  Rust 后端（`--features fault` 出故障自测变体）；Cargo.toml pin `zeroize`/`zeroize_derive`/`base64ct`
  以兼容 Rust 1.83。**Rust↔OpenSSL 11000 向量全一致**（seed 42×6000 + 777×5000）；负向自测升
  **30/30**（新增 Rust 跨语言 DIFF_mismatch 故障注入）。build_all + Dockerfile（装 rustup 1.83）已接入。
  全环境 Docker 化（clang+cmake+Go 1.22+Rust 1.83），便于迁移。本阶段 0 新发现。
